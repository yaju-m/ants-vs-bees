...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if not isinstance(insect, QueenAnt) or not insect.true_queen or not insect.armor:
                insect.place = None
        else:
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        elif self.armor > 0 and self.place.exit:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        if self.min_range:
            while place_num < self.min_range and curr_place is not hive:
                curr_place = curr_place.entrance
                place_num += 1

        while not curr_place.bees and curr_place is not hive:
            curr_place = curr_place.entrance
            place_num += 1

        if self.max_range:
            if place_num > self.max_range:
                return None

        if curr_place is not hive:
...
class ScubaThrower(ThrowerAnt):
    ...
    A ScubaThrower will not lose its armor when placed in Water."""
    name = 'Scuba'
    food_cost = 6
    watersafe = True
    implemented = True
        ...
        if not self.true_queen:
            ...
            while curr_place.exit:
                ...
                if curr_place.ant:
                    ...
                    if not ant_in_place.doubled:
                        ant_in_place.damage = ant_in_place.damage*2
                        ant_in_place.doubled = True
                    if ant_in_place.container and ant_in_place.ant:
                        ant_in_container = ant_in_place.ant
                        if not ant_in_container.doubled:
                            ant_in_container.damage = ant_in_container.damage*2
                            ant_in_container.doubled = True
